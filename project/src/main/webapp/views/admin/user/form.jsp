    ...
    <form class="layui-form" lay-filter="userForm">
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                ...
                <a class="layui-btn layui-btn-primary" href="${pageContext.request.contextPath}/admin/user/list">返回</a>
            </div>
        </div>
    </form>
</div>
...
<script>
    layui.use(['form','layer'], function(){
        ...
        form.on('submit(saveUser)', function(data){
            var vals = data.field;
            var url = vals.userId ? '${pageContext.request.contextPath}/admin/user/update' : '${pageContext.request.contextPath}/admin/user/save';
            fetch(url, {
                method:'POST',
                headers:{'Content-Type':'application/x-www-form-urlencoded'},
                body: Object.keys(vals).map(k=>encodeURIComponent(k)+'='+encodeURIComponent(vals[k]||'')).join('&')
            }).then(r=>r.json()).then(res=>{
                if(res.success){ layer.msg('保存成功',{icon:1}); setTimeout(()=>location.href='${pageContext.request.contextPath}/admin/user/list',800); }
